...
class Playground():
    ...
    def n_actions(self):  # action = [0~num_angles-1]
        return len(self.q_learner.actions)
    ...
    def isAtDestination(self):
        ...
        return cpos.isInRect(self.destination_line.p1, self.destination_line.p2)
    ...
    def predictAction(self, state):
        ...
        不需使用此function。
        '''
        return self.q_learner.predict(state)
    ...
    def step(self, action, update=True):
        ...
        origin_distance_to_goal = self.car.getPosition('center').distToLine2D(self.destination_line)
        ...
        if reward == 100:
            self.q_learner.epsilon *= 0.5
            self.q_learner.lr *= 0.5

        return self.state

    def compute_reward(self, origin_state: tuple | list, current_state: tuple | list, approach_goal: bool, action: float):
        if self.done:
        ...
        # 方向盤角度懲罰
        action_penalty = 0
        if origin_state[1] - origin_state[2] > 0:
            # 車子應該右轉
            if action < 0:
                action_penalty -= abs(action) * 0.5
            else:
                action_penalty += abs(action) * 0.25

        elif origin_state[1] - origin_state[2] < 0:
            # 車子應該左轉
            if action > 0:
                action_penalty -= abs(action) * 0.5
            else:
        ...
        # 距離障礙物過近懲罰（感測器值小表示離障礙物近）
        safe_distance = 8  # 單位依據感測器最大量程設計
        danger_penalty = 0
        for sensor in current_state:
            if sensor < safe_distance:
                danger_penalty += (safe_distance - sensor) * 2  # 越近懲罰越重

        # 鼓勵往目標前進（距離終點越近越好）
        distance_reward = 0 if approach_goal else -1

        return distance_reward - danger_penalty + action_penalty
